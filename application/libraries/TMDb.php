...
class TMDb
{
	 ...
	 * @return void
	 */
	public function __construct($apikey = '93a3f01925642df2c7898dc0d9fedcc7', $defaultFormat = TMDb::JSON, $defaultLang = 'en')
	{
		$this->setApikey(self::apikey);
	 ...
	 * @return mixed
         *
         * genre browsing needs genre ids
	 */
	public function browseMovies($order_by, $order, $params = array(), $format = null)
	{
		...
		{
                        $test['order_by'] = $order_by;
                        $test['order'] = $order;
                        $test['params'] = $params;

			$params['order_by'] = $order_by; //messes with the original string
			$params['order'] = $order;       //doesnt even make it an array
			return $this->_makeCall('Movie.browse', $test, $format);
		}
		else
		{
			return FALSE;
	...
	private function _makeCall($function, $param, $format, $method = TMDB::GET)
	{
		...
		if($method == TMDB::GET)
		{
			if(is_array($param) AND ! empty($param))
			{
                                ...
                                $paramnew['order_by'] = $param['order_by'];
                                $paramnew['order'] = $param['order'];

				$params .= '?'.http_build_query($paramnew).$paramtest;
			}
			elseif($param != '')
			{
				$arr = explode('/', $param);
